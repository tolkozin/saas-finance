...
with st.sidebar.expander("Scenarios, Taxes & Settings", expanded=True):
    scenario_sensitivity = st.slider("Scenario Sensitivity (+/- %)", -50, 50, 0, help="Настройка чувствительности сценария (влияет на конверсии и удержание).")
    corporate_tax = st.number_input("Corporate Tax %", value=1.0, help="Налог на прибыль. По умолчанию 1% (Грузия), возможны варианты Армения 5%, Казахстан 3-10%.")

with st.sidebar.expander("Payment Gateways & Transaction Fees", expanded=True):
    store_split = st.slider("Store vs. Web Split % (Store)", 0, 100, 50, help="Доля пользователей, покупающих через Store (остальные через Web).")
    web_split = 100 - store_split
    app_store_comm = st.number_input("App Store Commission %", value=15.0, help="15% по Apple Small Business Program до $1M, далее 30%.")
    web_comm_pct = st.number_input("Web Payment Commission %", value=3.5, help="Стандартные условия Lemon Squeezy / Paddle. Налоги Sales Tax они берут на себя.")
    web_comm_fixed = st.number_input("Web Payment Fixed Fee $", value=0.50, help="Фиксированная комиссия за транзакцию в Web.")
    bank_fee = st.number_input("Banking Transaction Fee %", value=1.0, help="Закладываем 1% на последующие банковские операции и конвертации.")

with st.sidebar.expander("Acquisition & Scaling", expanded=True):
    paid_budget = st.number_input("Initial Monthly Ad Budget ($)", value=150000.0, help="Начальный ежемесячный бюджет на рекламу.")
    ad_growth = st.number_input("Ad Budget MoM Growth %", value=5.0, help="Ежемесячный рост рекламного бюджета в процентах.")
    base_cpi = st.number_input("CPI (Cost Per Install) $", value=7.5, help="Базовая стоимость установки.")
    cpi_deg = st.number_input("CPI Degradation %", value=1.0, help="На сколько процентов увеличивается CPI за каждую добавленную $1,000 к бюджету.")
    conv_trial = st.number_input("Conv. to Trial %", value=25.0, help="Конверсия из установки в триал.")
    conv_paid = st.number_input("Trial-to-Paid %", value=25.0, help="Конверсия из триала в платную подписку.")

with st.sidebar.expander("Monetization & Retention", expanded=True):
    mix_weekly = st.number_input("Mix: Weekly %", value=0.0, help="Доля еженедельных подписок.")
    mix_monthly = st.number_input("Mix: Monthly %", value=48.0, help="Доля ежемесячных подписок.")
    mix_annual = st.number_input("Mix: Annual %", value=52.0, help="Доля ежегодных подписок.")
    ...
    price_weekly = st.number_input("Price: Weekly $", value=4.99, help="Цена еженедельной подписки.")
    price_monthly = st.number_input("Price: Monthly $", value=7.99, help="Цена ежемесячной подписки.")
    price_annual = st.number_input("Price: Annual $", value=49.99, help="Цена ежегодной подписки (оплата сразу).")
    
    churn_web = st.number_input("Monthly Churn: Web %", value=10.0, help="Ежемесячный отток пользователей Web.")
    churn_store = st.number_input("Monthly Churn: Store %", value=8.0, help="Ежемесячный отток пользователей Store.")

with st.sidebar.expander("Cost Structure", expanded=True):
    pre_salaries = st.number_input("Pre-Launch Salaries ($)", value=17475.0, help="Дизайн 3250 + RN разработка 11200 + Бэкенд 2200 + QA 825.")
    pre_setup = st.number_input("Pre-Launch Setup Costs ($)", value=8419.0, help="Интеграции 1540 + Контент/Статьи 4300 + Инфраструктура 225 + Непредвиденные 2354.")
    seed_investment = st.number_input("Seed Investment ($)", value=100000.0, help="Начальные инвестиции.")
    ongoing_salaries = st.number_input("Ongoing Monthly Salaries ($)", value=1200.0, help="Поддержка после запуска: Senior RN 700-840/мес + Дизайнер по задачам 200-400/мес.")
    platform_subs = st.number_input("Platform Subscriptions ($)", value=250.0, help="RevenueCat (от $99/мес при >$2.5K MTR) + AppsFlyer (от $150/мес при >10k конверсий).")
    cogs_per_user = st.number_input("COGS per Active User ($)", value=0.10, help="Затраты на сервера/хостинг на одного активного пользователя в месяц.")
...
def run_model(sens_percent):
    ...
    df["Ad Budget"] = paid_budget * ((1 + ad_growth / 100.0) ** (df["Month"] - 1))
    df["CPI"] = base_cpi * (1 + (cpi_deg / 100.0) * ((df["Ad Budget"] - paid_budget) / 1000.0).clip(lower=0))
    df["Installs"] = df["Ad Budget"] / df["CPI"]
    ...
    df["New Paid Users"] = df["New Trials"] * actual_conv_paid
    ...
    df["Marketing"] = df["Ad Budget"]
    df["Salaries"] = ongoing_salaries
    df["Platform Subs"] = platform_subs
    ...
    initial_cash = seed_investment - pre_salaries - pre_setup
...
month_range = st.slider("Select Month Range", 1, 60, (1, 60), help="Глобальный фильтр по месяцам. Влияет на все графики и метрики.")
...
col1.metric("Total Revenue", f"${f_df['Total Gross Revenue'].sum():,.0f}", help="Суммарная валовая выручка за выбранный период.")
col2.metric("Net Profit", f"${f_df['Net Profit'].sum():,.0f}", help="Суммарная чистая прибыль за выбранный период.")
col3.metric("End MRR", f"${f_df['Total MRR'].iloc[-1]:,.0f}", help="Регулярная месячная выручка на конец выбранного периода.")
col4.metric("Avg LTV/CAC", f"{f_df['LTV/CAC'].mean():.2f}x", help="Среднее отношение пожизненной ценности клиента к стоимости привлечения за период.")
...
with tab1:
    ...
    with c1:
        ...
        st.markdown("*Разделение выручки по когортам тарифов.*")
    ...
    with c2:
        ...
        st.markdown("*Помесячный баланс наличных средств с учетом предзапускных трат ~$25.8k. Ключевой график для оценки Burn Rate.*")
    ...
    with c3:
        ...
        st.markdown("*Сколько денег съедают комиссии Apple (15-30%) по сравнению с Lemon Squeezy (3.5% + $0.50).*")
    ...
    with c4:
        ...
        st.markdown("*Визуальное сравнение оттока пользователей с разных платформ.*")

with tab2:
    c5, c6 = st.columns(2)
    with c5:
        st.subheader("Payback Funnel: LTV vs Paid CAC vs Blended CAC")
        fig3 = px.line(f_df, x="Month", y=["LTV", "Paid CAC", "Blended CAC"], title="LTV vs CAC")
        st.plotly_chart(fig3, use_container_width=True)
        st.markdown("*Показывает соотношение стоимости привлечения и LTV.*")
        
    with c6:
        st.subheader("ROAS & Payback Period")
        fig5 = px.line(f_df, x="Month", y="Payback Period (Months)", title="Payback Period (Months)")
        st.plotly_chart(fig5, use_container_width=True)
        st.markdown("*На какой месяц после привлечения когорта окупает затраты на рекламу.*")
    ...
    with c7:
        ...
        st.markdown("*Отношение общей выручки к затратам на маркетинг.*")
...
with tab3:
    ...
    with c9:
        ...
        st.markdown("*Как Валовая выручка превращается в Чистую прибыль.*")
    ...
    with c10:
        ...
        st.markdown("*Сравнение итоговой прибыли для сценариев.*")

# --- FINANCIAL REPORTS ---
st.markdown("---")
st.header("Financial Reports")

rep_tab1, rep_tab2, rep_tab3, rep_tab4 = st.tabs(["P&L", "Cash Flow", "Balance Sheet", "Key Metrics"])

def display_df(df_to_show):
    st.dataframe(
        df_to_show,
        column_config={
            "Month": st.column_config.NumberColumn("Month", help="Месяц моделирования"),
            "Recognized Revenue": st.column_config.NumberColumn("Recognized Revenue", help="Признанная выручка (MRR)"),
            "Net Profit": st.column_config.NumberColumn("Net Profit", help="Чистая прибыль: EBITDA - Налог на прибыль"),
            "Cash Balance": st.column_config.NumberColumn("Cash Balance", help="Остаток денежных средств на конец месяца"),
            "Deferred Revenue": st.column_config.NumberColumn("Deferred Revenue", help="Отложенная выручка (Сумма собранных денег за год минус признанная выручка)"),
            "LTV": st.column_config.NumberColumn("LTV", help="Пожизненная ценность клиента: ARPU * Gross Margin / Churn"),
            "Blended CAC": st.column_config.NumberColumn("Blended CAC", help="Стоимость привлечения: Marketing / New Paid Users")
        },
...
with rep_tab1:
    ...
    display_df(f_df[["Month", "Recognized Revenue", "COGS", "Gross Profit", "Marketing", "Salaries", "Platform Subs", "Total Commissions", "EBITDA", "Corporate Tax", "Net Profit"]])
    
with rep_tab2:
    st.subheader("Cash Flow Statement")
    display_df(f_df[["Month", "Total Gross Revenue", "Total Commissions", "Total Expenses", "Corporate Tax", "Net Cash Flow", "Cash Balance"]])
    
with rep_tab3:
    st.subheader("Balance Sheet (Simplified)")
    display_df(f_df[["Month", "Cash Balance", "Deferred Revenue"]])
    
with rep_tab4:
    st.subheader("Key Metrics")
    display_df(f_df[["Month", "Total Active Users", "ARPU", "Blended Churn", "LTV", "Blended CAC", "LTV/CAC", "MER", "Payback Period (Months)"]])
